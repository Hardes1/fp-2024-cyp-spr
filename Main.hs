...
mapPlanetYearsToEarthYears :: Planet -> Float
mapPlanetYearsToEarthYears planet = case planet of 
   Mercury -> 0.2408467
   Venus ->  0.61519726
   Earth ->  1.0
   Mars ->  1.8808158
...
isLeapYear :: Int -> Either Bool String
isLeapYear year
  | year >= 0 = Left (((year `mod` 4 == 0) && (year `mod` 100 /= 0)) || (year `mod` 400 == 0))
  | otherwise = Right ("Year is negative" ++ show year)
